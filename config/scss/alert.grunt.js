...
module.exports = function(grunt) {
  ...
  var compileIcomoon = function() {
    var done = this.async();
    var concatScss = new ConcatScss();
    var options = {
      src: './scss/alert/icomoon/style.css',
      dest: './scss/alert/_icon-fonts.scss',
      rootDir: rootDir,
      addAutoGeneratedComment: true
    };
    concatScss.concat(options).then((result) => {
      done();
    }).catch((err) => { throw err; });
  };
  grunt.registerTask('compileIcomoon', "Compile font icons", compileIcomoon);

  grunt.registerTask("build", ['compileIcomoon', 'sass', 'postcss']);
